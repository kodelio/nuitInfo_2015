...
if ((isset($_POST['CriseID'])) AND ($_POST['CriseID']!="")){
  if ((isset($_POST['nameOperation'])) AND ($_POST['nameOperation']!="")){
    ...
    $myquery2 = $bdd->prepare("INSERT INTO operation (ID_USER, NOM) VALUES (?,?);");
    $myquery2->execute(array($_SESSION['IDUSER'],$_POST['nameOperation']));
    $myquery3 = $bdd->prepare("Select ID_OPERATION from operation where NOM=?;");
    $myquery3->execute(array($_POST['nameOperation']));
    $row = $myquery3->fetch();
    $myquery4 = $bdd->prepare("INSERT INTO asso_operation_crise (ID_CRISE, ID_OPERATION) VALUES (?,?);");
    $myquery4->execute(array($_POST['CriseID'],$row['ID_OPERATION']));
  }
}
if(isset($_SESSION['TYPEUSER']) && $_SESSION['TYPEUSER'] == 2)
{
  ...
  <head>
    ...
    <title>Alt-F4 | Formulaire d'opérations</title>
    ...
    <div class="box box-info">
      ...
      <form class="form-horizontal" method="POST">
        <div class="box-body">
         ...
         <div class="form-group">
          <label for="CriseID" class="col-sm-2 control-label">Crise</label>
          <div class="col-sm-9">
            <select class="form-control" id="CriseID" name="CriseID" required>
              <?php
              $bdd = new PDO('mysql:host=localhost;dbname=rgdyprykza;charset=utf8', 'rgdyprykza', 'rRv2tVZK6P');
              $myquery = $bdd->prepare("Select ID_CRISE, NOM from crise;");
              $myquery->execute(array());
              while($row = $myquery->fetch()){?>
              <option value="<?php echo $row['ID_CRISE']; ?>"><?php echo $row['NOM']; ?></option>
              <?php }
              ?>
            </select>
          </div>
